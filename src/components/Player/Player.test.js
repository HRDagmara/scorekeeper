...
it('should call onPlayerScoreChange with -1 when minus button is clicked', () => {
    ...
    const minusButton = playerComponent.find('.Player__button').second();
...
it('should call onPlayerRemove with x when button is clicked', () => {
    const mockedOnPlayerRemove = jest.fn();
    const playerComponent = shallow(<Player onPlayerRemove={mockedOnPlayerRemove} />);
  
    const removeButton = playerComponent.find('.Player__button').third();
  
    removeButton.simulate('click');
  
    expect(mockedOnPlayerRemove).toBeCalledWith(x);
});
